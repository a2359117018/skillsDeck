...
**完成日期**：2026-05-28
**关联提交**：`7563d4c`, `d753fb4`, `400b6a8`, `3fb1e3a`, `01e7086`, `03adcd9`, `15c0dd2`
...
| P2-19 | 统一 `SkillInstallDialog` 与 `useSkillInstall` | `SkillInstallDialog.vue` + `useSkillInstall.ts` | `useAgentSelection` composable | ✅ |
| P2-20 | 清理 `shared/types.ts` | `shared/types.ts` | 删除 Skill 接口、迁移工具函数 | ✅ |
| P2-21 | 提取共享路径工具函数 | 3 个文件 | `expandTildePath()` | ✅ |

**验证结果**：`npm run build` ✅ | `npm run typecheck` ✅ | `npm run lint` ✅ | git tag `p2-complete` ✅
...
**完成日期**：2026-05-28
**关联提交**：`d401ac9`, `4483840`

| # | 问题 | 涉及文件 | 交付物 | 状态 |
|---|------|---------|--------|------|
| P3-22 | 引入依赖注入容器（可选） | — | 评估后决定跳过 | ⏭️ 跳过 |
...
**P3-22 评估结论**：当前服务层约 10 个类，依赖关系简单，无循环依赖，全部为单例。项目无测试基础设施，引入 DI 容器的边际收益远低于其带来的复杂度和样板代码。决定跳过。

**P3-23 额外改进**：在替换解压库的同时，补充了 `isPathInside` zip-slip 验证和 `path.normalize` subPath 加固，使 `GitHubSkillInstaller` 的安全模式与 `ArchiveSkillInstaller` 一致。

**P3-24 额外改进**：提取了 `UseCachedResourceOptions` 命名接口，补充了 JSDoc 文档，TTL 过期时正确标记 `isStale`。

**验证结果**：`npm run build` ✅ | `npm run typecheck` ✅ | `npm run lint` ✅ | git tag `p3-complete` ✅
